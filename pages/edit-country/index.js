...
import { EDIT_COUNTRY } from '../../graphql/queries';
...
export default function index() {
  ...
  // call useEffect only if router.query isn't empty
  React.useEffect(() => {
    if (router.query.name) {
      setName(router.query.name);
      setYear(router.query.year);
      setArea(router.query.area);
      setPopulation(router.query.population);
    }
  }, [router.query]);
  ...
  const [editCountry] = useMutation(EDIT_COUNTRY, {
    ...
    onError: async (error) => {
      if (error.networkError) {
        const err = getNetworkErrors(error.networkError.result.errors) || 'Something went wrong!';
        setError(err);
      } else {
